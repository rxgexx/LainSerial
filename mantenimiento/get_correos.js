...
const { infoburo } = require("../bot/api/api_Persona.js");
...
module.exports = (bot) => {
  bot.onText(/[\/.$?!]correos (.+)/, async (msg, match) => {
    //POLLING ERROR
    ...
    try {
      //RESPONSE TITULAR
      const responseTitular = await infoburo(dni);
      const data = responseTitular.listaAni.correos;

      if (data.length === 0) {
        await bot.deleteMessage(chatId, consultandoMessage.message_id);
        const yx = `*[ ✖️ ] No se encontró registros de correos* para el DNI \`${dni}\`.`;

        bot.sendMessage(chatId, yx, messageOptions);
      } else {
        const persona = responseTitular.listaAni.generales;
        ...
        let telRes = `*[#LAIN-DOX 🌐] ➤ #CORREOS*\n\n`;
        telRes += `*[ ☑️ ] NUMEROS DE* - \`${dni}\` -\n\n`;
        telRes += `*➤ INF. PERSONA:*\n`;
        telRes += `  \`⌞\` *DNI:* \`${persona.documento}\`\n`;
        telRes += `  \`⌞\` *TITULAR:* \`${titular}\`\n`;
        telRes += `  \`⌞\` *UBICACION:* \`${persona.lugar_nacimiento}\`\n`;
        telRes += `  \`⌞\` *NACIMIENTO:* \`${persona.nacimiento}\`\n\n`;

        telRes += `*➤ BASE DE CORREOS:*\n\n`;

        //SI LOS NÚMEROS SON MENOR O IGUAL A 10 RESULTADOS
        if (data) {
          //POR CADA DATO
          data.forEach((dato, i) => {
            const number = dato.correo;
            const index = i + 1;

            telRes += `  \`⌞\` *INDEX:* \`${index}\`\n`;
            telRes += `  \`⌞\` *CORREO:* \`${number}\`\n\n`;
          });
