//APIS
const { apiName_2 } = require("../api/api_Nombres.js");
...
module.exports = (bot) => {
  bot.onText(/[\/.$?!]nm2 (.+)/, async (msg, match) => {
    //POLLING ERROR
    ...
    if (match && match[1]) {
      ...
      if (!input.includes("|")) {
        ...
        replyToUsoIncorrecto += `*➜ EJEMPLO:* \`/nm2 Pedro|Castillo|Terrones\``;
    ...
    try {
      ...
      const edadMinima = commandArgs[3];
    //   console.log("EDAD MINIMA: ", edadMinima);
      const edadMaxima = commandArgs[4];
    //   console.log("EDAD MAXIMA: ", edadMaxima);
      ...
      //SIN DATOS
      const noDatos =
        responseNombres.error === "El parámetro 'dni' es requerido.";
      ...
      if (noDatos) {
        ...
        const nombresData = responseNombres.data;
