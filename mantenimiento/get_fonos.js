...
module.exports = (bot) => {
  bot.onText(/[\/.$?!]fonos (.+)/, async (msg, match) => {
    ...
    try {
      ...
      const data = responseTitular.listaAni.telefonos;
      ...
      if (data.length === 0) {
        ...
        //RESPONSE TITULAR
        const titular = persona.nombres + " " + persona.paterno + " " + persona.materno;

        //CONSTRUCCIÓN DEL MENSAJE
        let telRes = `*[#LAIN-DOX 🌐] ➤ #FONOS*\n\n`;
        ...
        if (data.length <= 6) {
          ...
          data.forEach((dato) => {
            ...
            const operator = dato.origen_data;
            const periodo = dato.fecha_data;
