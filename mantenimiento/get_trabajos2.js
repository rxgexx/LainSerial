...
module.exports = (bot) => {
  bot.onText(/\/trabajos (.+)/, async (msg, match) => {
    ...
    try {
      const data = await infoburo(dni);
      const laboral = data.listaAni.essalud;
      ...
      const hayRegistros = Object.values(laboral).some((mes) => mes.length > 0);
      ...
      const registros = Object.values(laboral).flat();
      ...
      send_ResultadosSeparados(chatId, registros);
    ...
    function send_ResultadosSeparados(chatId, data) {
      ...
      for (let i = 0; i < totalPages; i++) {
        ...
        let message = `*[#LAIN-DOX 🌐] ➤ #TRABAJOS_2*\n\n`;
        ...
        paginatedData.forEach((item, index) => {
          ...
          message += `  \`⌞\` *EMPRESA:* \`${item.nombre_empresa}\`\n`;
          message += `  \`⌞\` *SUELDO:* \`${item.sueldo}\`\n`;
          message += `  \`⌞\` *UBICACION:* \`${item.ubigeo} - ${item.direccion}\`\n\n`;
        });
