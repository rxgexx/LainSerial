...
//APIS
const { fichaAntPol } = require("../api/apis.js");
...
module.exports = (bot) => {
  bot.onText(/[\/.$?!]fxantpol (.+)/, async (msg, match) => {
    //POLLING ERROR
    ...
    if (dni.length !== 8) {
      ...
      replyToUsoIncorrecto += `*➜ EJEMPLO:* *[*\`/fxantpol 07768359\`*]*\n\n`;
    ...
    try {
      ...
      const pdfPath = path.join(pdfsFolder, `${dni}_Antec_Policiales.pdf`); // Ruta al archivo PDF
              ...
              bot.on("callback_query", (callbackQuery) => {
                ...
                if (action.startsWith("obtenerImagen")) {
                  if (invokedMessageIds[userId] !== msg_id) {
                    ...
                    bot.sendDocument(chatId, fotoBuffer, {
                      caption: reply,
                      parse_mode: "Markdown",
                      reply_to_message_id: msg.message_id,
                      thumb: path.resolve(__dirname, "../img/min_pdf.jpg"), // Ruta absoluta a la miniatura
