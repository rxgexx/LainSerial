...
//APIS
const { fichaAntJud } = require("../api/apis.js");
...
module.exports = (bot) => {
  bot.onText(/[\/.$?!]fxantjud (.+)/, async (msg, match) => {
    //POLLING ERROR
    ...
    const y = `*[ ⚙️ ] Construyendo* los \`ANTECEDENTES JUDICIALES\` del *➜ DNI* \`${dni}\``;
    ...
    try {
      ...
      const fotoImagen = responsefichaAntJud.fotoImagen;

      //Declarate the path where save the pdf's
      const pdfsFolder = path.join(__dirname, "../../fichasDocuments"); // Ruta a la carpeta "docs"
      const pdfPath = path.join(pdfsFolder, `${dni}_Antec_Judiciales.pdf`); // Ruta al archivo PDF
