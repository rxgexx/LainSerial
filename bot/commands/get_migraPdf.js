...
const img = path.join(__dirname, "../img/migraMiniatura.jpg");
const dirDoc = path.join(__dirname, "../../fichasDocuments/migracionesPdf");
...
module.exports = (bot) => {
  bot.onText(/[\/.$?!]migrapdf (.+)/, async (msg, match) => {
    ...
    if (dni.length !== 8) {
      let replyToUsoIncorrecto = `*[ ✖️ ] Uso incorrecto*, utiliza *[*\`/migrapdf\`*]* seguido de un número de *dni* de \`8 dígitos\`\n\n`;
      replyToUsoIncorrecto += `*➜ EJEMPLO:* *[*\`/migrapdf 06256217\`*]*\n\n`;
    ...
    let yx = `*[ 💬 ] Consultando* \`REPORTE MIGRATORIO\` en *PDF del* *➜ dni* \`${dni}\``;
    ...
    try {
      const datos = await migracionesPdf(dni);

      if (
        datos.data.status === false &&
        datos.data.mensaje === "No se encontraron datos para el DNI consultado"
      ) {
      ...
      // Datos obtenidos
      const pdf = datos.data.Migraciones.datapdf;
      ...
      const dataPropietario = datos.data.Migraciones.dataPersona;

      const apepaterno = dataPropietario.apepaterno;
      const apematerno = dataPropietario.apematerno;
      const nombres = dataPropietario.nombres;
      const painacionalidad = dataPropietario.painacionalidad;
      const fecnacimiento = dataPropietario.fecnacimiento;

      // Datos dni

      let caption = `*[#LAIN-DOX 🌐] ➤ #MIGRACIONESPDF*\n\n`;

      caption += `*[ ☑️ ]  REPORTE MIGRATORIO -* \`${dni}\` *- ✈*\n\n`;

      caption += `*➤ CONSULTADO:*\n`;
      caption += `  \`⌞\` *NOMBRE:* \`${nombres}\`\n`;
      caption += `  \`⌞\` *AP. PATERNO:* \`${apepaterno}\`\n`;
      caption += `  \`⌞\` *AP. MATERNO:* \`${apematerno}\`\n`;
      caption += `  \`⌞\` *FE. NACIMIENTO:* \`${fecnacimiento}\`\n`;
      caption += `  \`⌞\` *PAIS. NACIMIENTO:* \`${painacionalidad}\`\n\n`;
      ...
      const filePath = path.join(dirDoc, `reporteMigratorio_${dni}.pdf`);
