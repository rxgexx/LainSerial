//SE REQUIRE LAS APIS
const { getReniecRes } = require("../api/apis.js");
...
module.exports = (bot) => {
  bot.onText(/[\/.$?!]dnir (.+)/, async (msg, match) => {
    //POLLING ERROR
    ...
    if (dni.length !== 8) {
      let replyToUsoIncorrecto = `*[ ✖️ ] Uso incorrecto*, utiliza *[*\`/dnir\`*]* seguido de un número de *CELULAR* de \`8 dígitos\`\n\n`;
      replyToUsoIncorrecto += `*➜ EJEMPLO:* *[*\`/dnir 48159191\`*]*\n\n`;
    ...
    try {
      const datosReniec = await getReniecRes(dni);
      const listaAni = datosReniec;
      ...
      function calcularEdad(fechaNacimiento) {
        ...
        const [anio, mes, dia] = fechaNacimientoSplit;
        const fechaNacimientoFormatoCorrecto = new Date(
          `${mes}/${dia}/${anio}`
        );
        ...
        let edad =
          ...
          fechaNacimientoFormatoCorrecto.getFullYear();

        if (
          fechaActual.getMonth() < fechaNacimientoFormatoCorrecto.getMonth() ||
          (fechaActual.getMonth() ===
            fechaNacimientoFormatoCorrecto.getMonth() &&
            fechaActual.getDate() < fechaNacimientoFormatoCorrecto.getDate())
        ) {
          edad--;
        }
      ...
      const nuEdad = calcularEdad(feNacimiento);

      let resDni = `*[#LAIN-DOX 🌐] ➤ #RENIECRESPALDO*\n\n`;
      ...
      resDni += `\`⌞\` *DEPARTAMENTO:* \`${depaDireccion}\`\n`;
      resDni += `\`⌞\` *PROVINCIA:* \`${provDireccion}\`\n`;
      resDni += `\`⌞\` *DISTRITO:* \`${distDireccion}\`\n`;
      resDni += `\`⌞\` *DIRECCIÓN:* \`${desDireccion}\`\n\n`;

      resDni += `*➜ 📧 EXTRAS*\n\n`;

      resDni += `\`⌞\` *PADRE:*  \`${nomPadre}\`\n`;
      resDni += `\`⌞\` *MADRE:* \`${nomMadre}\`\n`;
