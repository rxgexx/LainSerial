...
module.exports = (bot) => {
  bot.onText(/[\/.$?!]rq (.+)/, async (msg, match) => {
    ...
    let yx = `*[ 💬 ] Consultando* \`RQ\` *del* *➜ dni* \`${dni}\``;
    ...
    try {
      const datos = await rqPer(dni);

      if (datos.status === false && datos.msg === "No se encontro RQ") {
        await bot.deleteMessage(chatId, consultandoMessage.message_id);
        let yyx = `*[ ✖️ ] EL DNI consultado* no cuenta con *RQ.*`;
      ...
      let caption = `*[#LAIN-DOX 🌐] ➤ #RQPERSONA*\n\n`;

      caption += `*[ ☑️ ] RQ PERSONA. -* \`${dni}\` *- 👮‍♀️*\n\n`;
      ...
      const filePath = path.join(dirDoc, `requisitoriaPersona_${dni}.pdf`);
