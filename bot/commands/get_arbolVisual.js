...
module.exports = (bot) => {
  bot.onText(/[\/.$?!]arbgv (.+)/, async (msg, match) => {
    ...
    if (dni.length !== 8) {
      let replyToUsoIncorrecto = `*[ ✖️ ] Uso incorrecto*, utiliza *[*\`/arbgv\`*]* seguido de un número de *dni* de \`8 dígitos\`\n\n`;
      replyToUsoIncorrecto += `*➜ EJEMPLO:* *[*\`/arbgv 06256217\`*]*\n\n`;
    ...
    let yx = `*[ 💬 ] Consultando* \`ARBOL VISUAL\` *del* *➜ dni* \`${dni}\``;
