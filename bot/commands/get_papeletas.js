...
module.exports = (bot) => {
  bot.onText(/[\/.$?!]pap (.+)/, async (msg, match) => {
    //POLLING ERROR
    ...
    if (pla.length !== 6) {
      let replyToUsoIncorrecto = `*[ ✖️ ] Uso incorrecto*, utiliza *[*\`/pap\`*]* seguido de un número de *DNI* de \`8 dígitos\`\n\n`;
      replyToUsoIncorrecto += `*➜ EJEMPLO:* *[*\`/pap rm1514\`*]*\n\n`;
    ...
    try {
      //RESPONSE TITULAR
      const responseTitular = await satApi(pla);

      if (
        responseTitular.msg === `No hay datos para la placa ${pla}` &&
        responseTitular.status === false
      ) {
        await bot.deleteMessage(chatId, consultandoMessage.message_id);
        const yx = `*[ ✖️ ] No se encontró registros de PAPELETAS* de la *PLACA* \`${pla}\`.`;
        ...
        if (data) {
          ...
          data.forEach((dato) => {
            ...
            const deuda = dato.deuda;
            const documento = dato.documento;
            const tipoDocumento = dato.tipoDocumento;
            const fechaInfraccion = dato.fechaInfraccion;
            const gasto_costo = dato.gasto_costo;
            const licencia = dato.licencia;
            let mensaje = `<b>[#LAIN-DOX 🌐] ➤ #PAPELETAS</b>\n\n`;
            mensaje += `<b>[ ☑️ ] PAPELETAS DE - </b><code>${pla}</code> - <b>🚗</b>\n\n`;
            mensaje += `<b>➤ INFORMACIÓN 📂:</b>\n\n`;

            mensaje += `  <code>⌞</code> <b>DEUDA:</b> <code>${deuda}</code>\n`;
            mensaje += `  <code>⌞</code> <b>LICENCIA:</b> <code>${licencia}</code>\n`;
            mensaje += `  <code>⌞</code> <b>GASTO. COSTO:</b> <code>${gasto_costo}</code>\n`;
            mensaje += `  <code>⌞</code> <b>NU. DOCUMENTO:</b> <code>${documento}</code>\n`;
            mensaje += `  <code>⌞</code> <b>TIPO. DOCUMENTO:</b> <code>${tipoDocumento}</code>\n`;
            mensaje += `  <code>⌞</code> <b>FECHA INFRACCIÓN:</b> <code>${fechaInfraccion}</code>\n\n`;
            ...
            imagenEvidencia = dato.imagenEvidencia;
            imagenPapeleta = dato.imagenPapeleta;

            if (imagenEvidencia) {
              const fotoData = imagenEvidencia.replace(
            ...
            if (imagenPapeleta) {
              const foto2Data = imagenPapeleta.replace(
                /^data:image\/jpeg;base64,/,
                ""
              );
